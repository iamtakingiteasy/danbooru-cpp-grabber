...
void parseArguments(int argc, char ** argv) {
	...
	while(getopt_long_only(argc,argv,"",longOpts,&longIndex) != -1) {
		...
		if (strcmp(opt,"search-order") == 0
			...
			if (optarg) {
				if (false) {
					...
					log.error("Wrong search ordering '" + std::string(optarg) + "'\n");
				}
				if (strcmp(opt,"search-order-reverse") == 0 ||
					strcmp(opt,"sor") == 0) {
					conf.searchOrderReverse = true;
				}
			}
		}
		...
		if (strcmp(opt,"download-page") == 0 || strcmp(opt,"dp") == 0) {
			if (optarg) conf.pageOffset = atoi(optarg);
		}

		if (strcmp(opt,"list-all") == 0 || strcmp(opt,"la") == 0) {
			conf.action = GA_MODULE_LIST_ALL;
		}

		if (strcmp(opt,"list-downloaders") == 0 || strcmp(opt,"ld") == 0) {
			conf.action = GA_MODULE_LIST_DOWNLOADERS;
		}

		if (strcmp(opt,"list-boards") == 0 || strcmp(opt,"lb") == 0) {
			conf.action = GA_MODULE_LIST_BOARDS;
		}

		if (strcmp(opt,"list-handlers") == 0 || strcmp(opt,"lh") == 0) {
			conf.action = GA_MODULE_LIST_HANDLERS;
		}

		if (strcmp(opt,"working-directory") == 0 || strcmp(opt,"C") == 0) {
			if (optarg) conf.workingDirectory = optarg;
		}
	}
	for (int i = optind; i < argc; i++) {
		conf.tags.push_back(argv[i]);
	}
...
void validateParameters() {
	bool wasError = false;
	if (conf.action == GA_INVALID || conf.action == GA_LAST) {
		log.error("Wrong action.\n");
		wasError = true;
	}
	if (conf.searchOrder == SO_INVALID || conf.searchOrder == SO_LAST) {
		log.error("Wrong ordering.\n");
		wasError = true;
	}

	// Downloader
	if (conf.iDownloader.empty()) {
		if (modules.getDownloaders().empty()) {
			log.error("No downloader modules found!\n");
			wasError = true;
		} else {
			conf.downloader = modules.getDownloaders()[0].create();
		}
	} else {
		if (modules.downloaderWithNameExists(conf.iDownloader)) {
			
		} else {
			log.error("No downloader module iwth name " + conf.iDownloader + " found!\n");
			wasError = true;
		}
	}
	if (wasError) {
		log.warning("Error detected. Exiting.\n");
		exit(-1);
	}
